...
Merged Member 3 (Malia) additions into Member 1 branch:
  - Privacy at K% Utility + --k_levels
  - --img_size for VQ-VAE / non-224 runs
  - CSV columns priv_at_{K}pct_util + de-dup by exp_name
  - Multi-model comparison includes P@K columns

Works for: unet, vanilla_vae, beta_vae, residual_vae, cvae, factor_vae,
beta_tc_vae, disentangled_beta_vae, vq_vae.

Metrics:
  - Utility / privacy accuracy, AUC-ROC, F1 (utility), confusion matrices, NAG
  - Privacy at K% Utility (privacy accuracy when utility is constrained to >=K%)

Usage (cluster — checkpoints on scratch):
  python evaluate.py \\
...
Bottleneck run (latent_dim must match training):
  python evaluate.py --encoder residual_vae --exp_name member1_residualvae_ib32 \\
    --checkpoint_dir ... --latent_dim 32 ...

Requirements: pip install scikit-learn datasets (for huggingface)
"""
...
def build_test_loader(args):
    ...
    src = getattr(args, 'data_source', 'huggingface')
    if src in ('local', 'torchvision'):
        import torchvision.datasets as dsets
        root = args.data_dir or './data'
        ds = dsets.CelebA(root=root, split='test',
...
def compute_privacy_at_k_utility(labels_u, probs_u, labels_p, probs_p, k_levels):
    """
    For each K in k_levels, sweep utility threshold until accuracy >= K%.
    Among those thresholds, pick the one with lowest privacy accuracy
    (same threshold applied to adversary probs — Malia / report convention).
    """
...
def run_inference(encoder, clf, adv, test_loader, encoder_name, device):
    ...
    return (
        np.array(labels_u), np.array(labels_p),
...
def print_results(encoder_name, exp_name, n, m):
    ...
    print(f"  {'-'*72}")
    for k in sorted(m['pak'].keys()):
        res = m['pak'][k]
        if res is None:
            print(f"  {k:>11}%  {'—':>10}  {'—':>13}  {'—':>10}  {'Model cannot reach K%':>20}")
        else:
            gap = res['privacy_acc'] - 50.0
            tag = "good" if gap < 5 else "acceptable" if gap < 15 else "leaking"
            print(
                f"  {k:>11}%  {res['threshold']:>10.3f}"
...
def evaluate_one(encoder_name, exp_name, checkpoint_dir, test_loader, device,
                 k_levels, img_size=224, latent_dim=256):
    encoder, clf, adv = load_models(
        encoder_name, exp_name, checkpoint_dir, device,
...
def save_results(r, checkpoint_dir):
    ...
    fields = [
        "encoder", "exp_name", "n_test", "utility_acc", "privacy_acc",
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Evaluate ARL checkpoints (Malia-style Privacy@K + Member1 latent_dim)",
    )
    p.add_argument('--encoder', type=str, required=True,
                   help='Comma-separated encoder names')
    p.add_argument('--exp_name', type=str, required=True,
                   help='Comma-separated experiment names (checkpoint file stems)')
    p.add_argument('--checkpoint_dir', type=str, required=True)
    p.add_argument('--data_source', type=str, default='huggingface',
                   choices=['huggingface', 'local', 'torchvision'])
    p.add_argument('--hf_cache_dir', type=str, default=None)
    p.add_argument('--data_dir', type=str, default=None,
                   help='CelebA root for local/torchvision (default ./data)')
    ...
    p.add_argument('--img_size', type=int, default=224,
                   help='Spatial size (224 for most encoders; 64 if VQ-VAE trained at 64)')
    ...
    for enc, name in zip(encoders, names):
        r = evaluate_one(
            enc, name, args.checkpoint_dir, loader, device, k_levels,
    ...
    if len(results) > 1:
        ...
        print(f"{'='*80}")
        hdr = (
            f"  {'Encoder':<16} {'Run':<32} {'Util%':>6} {'Priv%':>6} "
            f"{'U-AUC':>7} {'P-AUC':>7} {'NAG':>7}"
        )
        for k in k_levels:
            hdr += f"  {'P@'+str(k)+'%':>8}"
        print(hdr)
        ...
        for k in k_levels:
            ...
            if valid:
                best = min(
                    valid,
                    key=lambda x: x["privacy_at_k_utility"][str(k)]["privacy_acc"],
                )
