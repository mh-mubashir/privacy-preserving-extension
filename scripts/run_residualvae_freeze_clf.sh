...
echo "== ResidualVAE --freeze_utility_clf_arl (professor: no smile-clf updates during ARL) =="
...
PYTHONPATH="$REPO_DIR" python "$REPO_DIR/adversarial_training.py" \
  ...
  --device          cuda \
  --exp_name        member1_residualvae_freeze_utility_arl \
  --num_epochs      13 \
  --warmup_epochs   3 \
  --batch_size      16 \
  ...
  --num_workers     0 \
  2>&1 | tee "$OUT_DIR/member1_residualvae_freeze_utility_arl.log"
