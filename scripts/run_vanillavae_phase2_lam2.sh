#!/bin/bash
#SBATCH --job-name=p2v_lam2
#SBATCH --partition=courses-gpu
#SBATCH --gres=gpu:1
#SBATCH --mem=16G
#SBATCH --cpus-per-task=2
#SBATCH --time=10:00:00
#SBATCH --output=/scratch/%u/pp_ext_member2/logs/vanillavae_p2_%j.out
#SBATCH --error=/scratch/%u/pp_ext_member2/logs/vanillavae_p2_%j.err

# VanillaVAE two-phase ARL training
...
echo "== VanillaVAE Two-Phase ARL Training =="
...
PYTHONPATH="$REPO_DIR" python "$REPO_DIR/adversarial_training.py" \
  ...
  --device cuda \
  --exp_name p2_vanillavae_lam2_13e_20k \
  --num_epochs 13 \
  --warmup_epochs 3 \
  --batch_size 16 \
  ...
  --num_workers 1 \
  2>&1 | tee "$OUT_DIR/p2_vanillavae_lam2_13e_20k.log"
