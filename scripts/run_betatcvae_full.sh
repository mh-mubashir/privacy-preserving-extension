#!/bin/bash
#SBATCH --job-name=m2_betatcvae
#SBATCH --partition=courses-gpu
#SBATCH --gres=gpu:1
#SBATCH --mem=16G
#SBATCH --cpus-per-task=2
#SBATCH --time=08:00:00
#SBATCH --output=/scratch/%u/pp_ext_member2/logs/betatcvae_full_%j.out
#SBATCH --error=/scratch/%u/pp_ext_member2/logs/betatcvae_full_%j.err

# Member 2 (Sindhu) — Full ARL training: BetaTCVAE encoder
...
echo "== BetaTCVAE Full ARL Training =="
...
PYTHONPATH="$REPO_DIR" python "$REPO_DIR/adversarial_training.py" \
  ...
  --device cuda \
  --exp_name member2_betatcvae_10e_20k \
  --num_epochs 10 \
  --batch_size 16 \
  ...
  --num_workers 1 \
  2>&1 | tee "$OUT_DIR/member2_betatcvae_10e_20k.log"
