#!/bin/bash
#SBATCH --job-name=m2_disbetavae
#SBATCH --partition=courses-gpu
#SBATCH --gres=gpu:1
#SBATCH --mem=16G
#SBATCH --cpus-per-task=2
#SBATCH --time=08:00:00
#SBATCH --output=/scratch/%u/pp_ext_member2/logs/disbetavae_full_%j.out
#SBATCH --error=/scratch/%u/pp_ext_member2/logs/disbetavae_full_%j.err

# Member 2 (Sindhu) — Full ARL training: DisentangledBetaVAE encoder
...
PYTHONPATH="$REPO_DIR" python "$REPO_DIR/adversarial_training.py" \
  ...
  --num_workers 1 \
  2>&1 | tee "$OUT_DIR/member2_disbetavae_10e_20k.log"
